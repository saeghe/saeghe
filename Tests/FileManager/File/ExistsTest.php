...
test(
    ...
    case: function () {
        ...
        assert_false(exists($file->stringify()), 'File/exists is not working!');
    }
);
...
test(
    ...
    case: function (Path $file) {
        assert_true(exists($file->stringify()), 'File/exists is not working!');

        return $file;
    },
    before: function () {
        $file = Path::from_string(root() . 'Tests/PlayGround/File');
        file_put_contents($file->stringify(), 'content');
